...
@login_required(login_url='adminlogin')
def admin_add_categories_views(request):
    categoryForm=forms.CategoryForm()
    if request.method=='POST':
        categoryForm=forms.CategoryForm(request.POST, request.FILES)
        if categoryForm.is_valid():
            categoryForm.save()
        return HttpResponseRedirect('admin-categorie')
    return render(request,'ecom/admin_add_categories.html',{'categoryForm':categoryForm})
...
@login_required(login_url='adminlogin')
def admin_delete_categorie_view(request,pk):
    categorie=models.Categorie.objects.get(id=pk)
    categorie.delete()
...
@login_required(login_url='adminlogin')
def update_categorie_view(request,pk):
    categorie=models.Categorie.objects.get(id=pk)
    categoryForm=forms.CategoryForm(instance=categorie)
    if request.method=='POST':
        categoryForm=forms.CategoryForm(request.POST,request.FILES,instance=categorie)
        if categoryForm.is_valid():
            categoryForm.save()
            return redirect('admin-categorie')
    return render(request,'ecom/admin_update_categorie.html',{'categoryForm':categoryForm})
...
def payment_success_view(request):
    ...
    for product in products:
        models.Orders.objects.get_or_create(customer=customer,product=product,status='En attente',email=email,mobile=mobile,address=address)
